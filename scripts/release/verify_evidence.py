...
Usage:
  python3 verify_evidence.py --evidence-dir docs/build-evidence
"""
import argparse
import os
import re
...
def extract_ticket_ids(pr_lines: list[str]) -> set[str]:
    """Extract EPAC-NNN ticket IDs from merge commit messages."""
    ids: set[str] = set()
    for line in pr_lines:
        ids.update(re.findall(r"EPAC-\d+", line, re.IGNORECASE))
    return {t.upper() for t in ids}


def get_evidenced_tickets(evidence_dir: str) -> set[str]:
    """Return ticket IDs that have at least one screenshot in docs/build-evidence/."""
    if not os.path.isdir(evidence_dir):
        return set()
    ids: set[str] = set()
    for fname in os.listdir(evidence_dir):
        ids.update(re.findall(r"EPAC-\d+", fname, re.IGNORECASE))
    return {t.upper() for t in ids}
...
def main() -> None:
    ...
    merged_tickets = extract_ticket_ids(pr_lines)
    evidenced_tickets = get_evidenced_tickets(args.evidence_dir)
