...
#
# Do not import the older member-speeches/epac-api-gateway statement as this resource. The live
...
All 10 desired bare-name production Lambda functions currently exist and should be imported. If a function is absent in a fresh account, Terraform will create it from `placeholder.zip`; code and environment remain deployment-workflow concerns. The retired `search`, `device-register`, and `live-status` functions are intentionally omitted from desired state; a human `terraform apply` removes their managed AWS resources after validation.
