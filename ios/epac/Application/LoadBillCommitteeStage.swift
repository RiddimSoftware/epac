...
/// upcoming meetings, and past meetings with witness counts when available.
/// Returns `nil` when the bill is not currently before a committee; the "In
/// committee" panel hides itself in that case (a UI-layer policy, per the
/// feature's boundary rule).
