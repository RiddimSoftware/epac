// config Lambda — GET /api/v1/config
package main

import (
	"context"
	"encoding/json"
	"net/http"

	"epac/observability"
	"epac/shared/artifacts"
	"github.com/aws/aws-lambda-go/events"
	"github.com/aws/aws-lambda-go/lambda"
)

const appConfigArtifactKey = "config/v1/app.json"

type AppConfig struct {
	MinimumSupportedVersion string          `json:"minimum_supported_version"`
	Features                map[string]bool `json:"features"`
}

var newArtifactStore = artifacts.NewFromEnv

func HandleRequest(ctx context.Context, _ events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	store, err := newArtifactStore(ctx)
	if err != nil {
		return jsonError(http.StatusServiceUnavailable, err.Error()), nil
	}
	data, err := store.Get(ctx, appConfigArtifactKey)
	if err != nil {
		status := http.StatusInternalServerError
		if artifacts.IsNotFound(err) {
			status = http.StatusNotFound
		}
		return jsonError(status, err.Error()), nil
	}
	var cfg AppConfig
	if err := json.Unmarshal(data, &cfg); err != nil {
		return jsonError(http.StatusInternalServerError, "invalid config artifact"), nil
	}
	if cfg.Features == nil {
		cfg.Features = map[string]bool{}
	}
	body, _ := json.Marshal(cfg)
	return events.APIGatewayV2HTTPResponse{
		StatusCode: http.StatusOK,
		Headers: map[string]string{
			"Content-Type":  "application/json",
			"Cache-Control": "public, max-age=300",
		},
		Body: string(body),
	}, nil
}

func jsonError(status int, message string) events.APIGatewayV2HTTPResponse {
	body, _ := json.Marshal(map[string]string{"error": message})
	return events.APIGatewayV2HTTPResponse{
		StatusCode: status,
		Headers:    map[string]string{"Content-Type": "application/json"},
		Body:       string(body),
	}
}

func main() {
	lambda.Start(observability.WrapAPIGatewayV2("config", HandleRequest))
}
