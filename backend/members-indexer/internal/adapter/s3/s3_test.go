...
func TestStoreUploadsSQLiteAndManifest(t *testing.T) {
	...
	store := NewStore(client, "bucket", "members/v1")

	hash, size, err := store.Upload(context.Background(), dbPath, "members/v1/index.sqlite")
	if err != nil {
	...
	if err := store.Write(context.Background(), domain.Manifest{Version: "v1", SQLiteKey: "members/v1/index.sqlite"}); err != nil {
	...
	if !strings.Contains(string(body), `"sqlite_key": "members/v1/index.sqlite"`) {
